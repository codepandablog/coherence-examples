...
public class SecurityExampleHelper {
	 ...
	 */
	public static Subject login(String sName) { // For simplicity, just create a
												// Subject. Normally, this would
												// be // done using JAAS.
		String sUserDN = "CN=" + sName + ",OU=Yoyodyne";
		Set setPrincipalUser = new HashSet();
		setPrincipalUser.add(new PofPrincipal(sUserDN)); // Map the user to a
															// role
	 ...
	 * role. * * @param sRoleRequired the role required for the operation * * @param
	 * subject the Subject requesting access * * @throws SecurityException if a
	 * Subject is null or does not have the * specified role Principal
	 */
